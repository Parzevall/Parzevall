...
portrait: colored key/value rows for system, work, stack, and hobbies -- NOT
GitHub stats (the contribution graph covers those).
...
ROWS = [
    ("host",),
    ("kv", "OS", "CachyOS Linux (arch, btw)"),
    ("kv", "Kernel", "Arch Linux, rolling"),
    ("kv", "Shell", "fish + Warp"),
    ("kv", "IDE", "Cursor, VS Code, terminal"),
    ("kv", "Uptime", "[REDACTED] years, no outages"),
    ("gap",),
    ("sec", "Work"),
    ("kv", "Day.Job", "machine learning engineer"),
    ("kv", "Night.Job", "full-time software developer"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "Python, JS/TS, some Go"),
    ("kv", "Formats", "HTML, CSS, JSON, YAML, MD"),
    ("kv", "Human", "English, fluent grep"),
    ("gap",),
    ("sec", "After hours"),
    ("bul", "building self-healing neural nets"),
    ("bul", "reading strangers' code for fun"),
    ("bul", "coffee, then coffee, then gym"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">parzival@oasis: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">parzival</tspan><tspan fill="{MUTED}">@</tspan>'
                 f'<tspan fill="{ACCENT}">oasis</tspan></text>'
                 f'<line x1="{KEY_X+118}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
