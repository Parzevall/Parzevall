"""
Prepare the portrait for clean ASCII conversion:
  1. remove the background (rembg) so the subject is isolated
  2. boost LOCAL contrast (CLAHE) so the flat-lit face and the near-white hair
     both gain internal structure -- this is what turns a blob into a face
...
Run once whenever the source photo changes; the ascii SVG itself is static.
    ...
    python scripts/prep_photo.py [input] [output.png]
"""
import os
import sys
