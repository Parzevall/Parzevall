...
portraits look noisy. One fill color + a good density ramp + high contrast (so
the background washes out to blank) reads as neat and legible.
...
portrait prints once and freezes. STATIC=1 emits the frozen state for previews.
"""
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "parzival-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">{USER}@{HOST}: ~$ ./portrait.sh</text>')
...
prompt = f"{USER}@{HOST}:~$ whoami "
char_w = 13 * 0.602  # approx advance width of 13px monospace
cursor_x = PAD + (len(prompt) + len(WHOAMI) + 1) * char_w
